...
class SimpleHTTPRequestHandler(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        r, info, file_id = self.deal_post_data()
        fpath = info
        ...
        if r:
            if contains_pan(fpath):
                ...
                f.write("<strong>Upload Successful!</strong>")
            else:
                f.write("<strong>Error! Could not detect PAN card. Please use better lighting and camera.</strong>")
        else:
    ...
    def deal_post_data(self):
        ...
        fn = os.path.join(path, 'uploads/', file_id + '.' + fn[0].split('.')[-1])
        line = self.rfile.readline()
        remainbytes -= len(line)
        line = self.rfile.readline()
        remainbytes -= len(line)
